...
class CrosswordsModelCrossword extends AdminModel {
	...
	public function getTable( $type = 'Crossword', $prefix = 'CrosswordsTable', $config = [] ) {
	...
	public function getItem( $pk = null ) {
		...
		if ( ! $pk )
		{
			...
			if ( $item = parent::getItem( $pk ) )
			{
				...
				$item->answers = $item->columns = [];
				$form_data     = Factory::getApplication()->getUserState( 'com_crosswords.edit.crossword.data' );
			}
		...
		elseif ( ! isset( $this->_item[$pk] ) )
		{
			try
			{
				...
				{
					$query->where( 'a.language in (' . $db->q( Factory::getLanguage()->getTag() ) . ',' . $db->q( '*' ) . ')' );
				}
				...
				if ( empty( $item ) )
				{
					return JError::raiseError( 404, Text::_( 'COM_CROSSWORDS_ERROR_POLL_NOT_FOUND' ) );
				}
				...
				if ( APP_VERSION >= 3 )
				{
					...
					if ( $assoc )
					{
						...
						if ( $item->id != null )
						{
							$associations = Associations::getAssociations( 'com_crosswords', '#__crosswords', 'com_crosswords.item', $item->id );
	...
	public function getForm( $data = [], $loadData = true ) {
		// Get the form.
		$form = $this->loadForm( 'com_crosswords.crossword', 'crossword', [ 'control' => 'jform', 'load_data' => $loadData ] );
		if ( empty( $form ) )
		{
			return false;
		}
	...
	protected function loadFormData() {
		...
		$app  = Factory::getApplication();
		$data = $app->getUserState( 'com_crosswords.edit.crossword.data', [] );
		...
		if ( parent::save( $data ) )
		{
			...
			if ( APP_VERSION >= 3 )
			{
				...
				if ( $assoc )
				{
					...
					{
						JError::raiseNotice( 403, Text::_( 'COM_CROSSWORDS_ERROR_ALL_LANGUAGE_ASSOCIATED' ) );
					}
	...
	public function featured( $pks, $value = 0 ) {
		...
		if ( empty( $pks ) )
		{
			$this->setError( Text::_( 'COM_CROSSWORDS_NO_ITEM_SELECTED' ) );
