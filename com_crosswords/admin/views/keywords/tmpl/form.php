...
<form class="form-horizontal" name="adminForm" id="adminForm" action="<?php echo Route::_( 'index.php?option=' . CW_APP_NAME . '&view=crosswords&task=save_crossword' ) ?>"
      method="post">

	<?php if ( $this->item->id > 0 ): ?>
        <div class="alert alert-error"><i class="icon icon-warning-sign"></i> <?php echo Text::_( 'COM_CROSSWORDS_KEYWORD_EDIT_HELP' ); ?></div>
	<?php endif; ?>

    <div class="control-group">
        <label class="control-label" for="keyword"><?php echo Text::_( 'COM_CROSSWORDS_FIELD_KEYWORD' ); ?>:<sup>*</sup></label>
        <div class="controls">
            <input type="text" name="keyword" id="keyword" class="input-xlarge" value="<?php echo $this->escape( $this->item->keyword ); ?>"
                   placeholder="<?php echo Text::_( 'COM_CROSSWORDS_FIELD_KEYWORD' ); ?>">
        </div>
    </div>

    <div class="control-group">
        <label class="control-label" for="question"><?php echo Text::_( 'COM_CROSSWORDS_FIELD_QUESTION' ); ?>:<sup>*</sup></label>
        <div class="controls">
			<textarea rows="3" cols="20" class="input-xlarge" id="question" name="question"
                      placeholder="<?php echo Text::_( 'COM_CROSSWORDS_FIELD_QUESTION' ); ?>"><?php echo $this->escape( $this->item->question ); ?></textarea>
        </div>
    </div>
    ...
    <div class="control-group">
        ...
        <div class="controls">
            <select name="published" id="published" size="1">
                ...
                <option value="0"<?php echo $this->item->published == 0 ? ' selected="selected"' : ''; ?>><?php echo Text::_( 'COM_CROSSWORDS_UNPUBLISHED' ); ?></option>
            </select>
        </div>
    </div>
