...
class JFormFieldModal_Crossword extends FormField {
	...
	protected function getInput() {
		...
		// Load language
		Factory::getLanguage()->load( 'com_crosswords', JPATH_ADMINISTRATOR );
		...
		if ( $allowClear && ! $scriptClear )
		{
			...
			$script[] = '		document.getElementById(id + "_name").value = "' . htmlspecialchars( Text::_( 'COM_CROSSWORDS_SELECT_A_CROSSWORD', true ),
					ENT_COMPAT, 'UTF-8' ) . '";';
		...
		if ( empty( $title ) )
		{
			$title = Text::_( 'COM_CROSSWORDS_SELECT_A_CROSSWORD' );
		}
		...
		$html[] = '<a class="modal btn hasTooltip" title="' . Text::_( 'COM_CROSSWORDS_CHANGE_CROSSWORD' ) . '"  href="' . $link . '&amp;' .
		          Session::getFormToken() . '=1" rel="{handler: \'iframe\', size: {x: 800, y: 450}}"><i class="icon-file"></i> '
		          . Text::_( 'JSELECT' ) . '</a>';
		...
		if ( $allowClear )
		{
			$html[] = '<button id="' . $this->id . '_clear" class="btn' . ( $value ? '' : ' hidden' ) .
			          '" onclick="return jClearCrossword(\'' . $this->id . '\')"><span class="icon-remove"></span> ' . Text::_( 'JCLEAR' ) . '</button>';
		}
